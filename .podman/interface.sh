...
case $choice in
  1)
    echo "Opening Neovim"
    echo ""
    podman run --rm -it -v $(pwd):/work -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v ~/.claude:/root/.claude:cached -v ~/.claude.json:/root/.claude.json ${image} bash /.nvim/entrypoint.sh
    ;;
  2)
    ...
    echo ""
    podman run --rm -it -v $(pwd):/work -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v ~/.claude:/root/.claude:cached -v ~/.claude.json:/root/.claude.json -p 127.0.0.1:${port}:8080 ${image} bash /.code-server/entrypoint.sh
    ;;
  3)
    echo "Entering an interactive terminal session"
    echo ""
    podman run --rm -it -v $(pwd):/work -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v ~/.claude:/root/.claude:cached -v ~/.claude.json:/root/.claude.json ${image}
    ;;
  *)
    echo "Invalid option."
    exit 1
